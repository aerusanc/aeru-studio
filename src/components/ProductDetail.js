...
const ProductImage = ({ src, alt }) => (
  <div className="w-full border rounded-lg sm:p-4 bg-white">
    <img
      src={src}
      alt={alt}
...
const ProductDetails = ({ name, reviews }) => (
  <div className=" w-full flex flex-col justify-items-center bg-white">
    <div className="bg-white sm:p-4 rounded-lg space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {['XS', 'S', 'M', 'L', 'XL', 'XXL'].map(size => (
          <button key={size} className="border border-black hover:bg-black hover:text-white rounded-sm px-2 sm:px-3 py-1 text-xs sm:text-sm">{size}</button>
        ))}
      </div>
      <div className="flex gap-2">
        <button className="px-4 py-2 border rounded-sm w-full border-black hover:bg-black hover:text-white font-bold text-xs sm:text-sm">
          SHOP NOW
        </button>
        <button className="p-2 border rounded-sm w-[45%] flex justify-center items-center border-black hover:bg-black hover:text-white font-bold text-xs sm:text-sm">
...
const ProductDetail = () => {
  ...
  return (
    <div className="bg-white min-h-screen overflow-hidden">
      <main className="mt-20 px-4 flex justify-center">
